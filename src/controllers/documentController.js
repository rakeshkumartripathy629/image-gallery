...
module.exports. createDocument = async (req, res) => {
  try {
    ...
    if (finduser.role === "ADMIN") {
      ...
      return res.status(201).send({status: true, message: "document Create Successfully",
        data: addDocument,
      });
    } else{
      return res.status(401).send({status: true, message: "Not Authorised",
      });
    }
  } catch (error) {
...
module.exports. getDocument = async function (req, res) {
  try {
    ...
    return res.status(200).send({ staus: true, msg: "All document", data: findDocument });
  } catch (error) {
    return res.status(500).send({ status: false, message: error.message });
  }
};
